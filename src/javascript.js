...
const mari = document.querySelector('.mari .axali')
...
async function add_to_dom_trending() {
    ...
    trending_el.innerHTML = data.slice(1, 3).map(e => {
        return `
            <div class="card" data-id="${e.id}">
                <div class="img">
                    <img src="${image_path + e.poster_path}">
...
get_mari_movies()
async function get_mari_movies() {
    const resp = await fetch(`https://api.themoviedb.org/3/trending/all/day?api_key=${API_KEY}`)
    const respData = await resp.json()
    return respData.results
}
...
add_to_dom_mari()
async function add_to_dom_mari() {

    const data = await get_mari_movies()
    console.log(data);

    mari.innerHTML = data.slice().map(e => {
        return `
            <div class="card" data-id="${e.id}">
                <div class="img">
                    <img src="${image_path + e.poster_path}">
